...
impl LBuilder {
  ...
  pub fn _ltree_normalize_pass_kont(&mut self, ltree: LExpr, kont: &mut dyn FnMut(&mut Self, LExpr) -> LExpr) -> LExpr {
    ...
    match &*ltree.term {
      ...
      &LTerm::AApp(ref idx_args, ref env) => {
    ...
    match self.hypotheses.pop_front() {
      ...
      Some(ref hyp) => {
        ...
        match &*hyp.ltree.term {
          ...
          &LTerm::AApp(ref arg_vars, ref env) => {
            let env_ty_v = self._tree_tyvar(env);
            self.constraints.push_back(TyConstraint::Eq(hyp.ty.clone(), Ty::AApp(arg_vars.clone(), TyRef::new(env_ty_v.clone()))));
            self.hypotheses.push_front(TyHypothesis{
              ltree:    env.clone(),
              ty:       env_ty_v,
              venv:     hyp.venv.clone(),
            ...
            match Self::_find_pty(substitution, env_ty_v) {
              ...
              Some(ref env_pty) => {
                match env_pty {
                  ...
                  _ => {
                    println!("DEBUG: unify failure: select key hash in a non-env");
                    return Err(());
                  }
                }
              }
